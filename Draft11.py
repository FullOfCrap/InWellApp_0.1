...
class PumpingTestFrame(HydroFrame):
    ...
    def create_widgets(self):
        ...
        self.add_row_button = tk.Button(self, text="Dodaj wiersz", command=self.add_row)
        ...
        self.data_table_scroll = tk.Scrollbar(self, orient="vertical")
        self.data_table_scroll.pack(side="right", fill="y")

        self.data_table = ttk.Treeview(self, yscrollcommand=self.data_table_scroll.set)
        self.data_table["columns"] = ("czas", "depresja_opad", "depresja_wznios")
        self.data_table.heading("#1", text="Czas")
        self.data_table.heading("#2", text="Depresja Opad")
        self.data_table.heading("#3", text="Depresja Wznios")
    ...
    def load_data(self):
        ...
        if file_path:
            try:
                self.data = pd.read_csv(file_path)  # Wczytaj dane z pliku CSV
    ...
    def draw_chart(self):
        if self.data is not None:
            ...
            punkt_przegiecia_wznios = self.find_inflection_point(self.data["Czas"], self.data["Depresja_roznica_wznios"])
    ...
    def add_row(self):
        # Tutaj dodaj kod do obsługi dodawania nowego wiersza do tabeli
        pass

    def edit_row(self):
        # Tutaj dodaj kod do obsługi edycji wybranego wiersza w tabeli
        pass
